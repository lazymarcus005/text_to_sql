# agentic_ai_system/validators/domain_guard.py
from __future__ import annotations
from dataclasses import dataclass
from typing import List, Tuple
import re


@dataclass(frozen=True)
class DomainGuardResult:
    allowed: bool
    message: str
    matched_keywords: List[str]


# โดเมนที่อนุญาต: 
_KEYWORD_GROUPS: List[List[str]] = [
    [
        "disaster", "emergency", "catastrophe",
        "flood", "flooding", "storm", "typhoon", "cyclone",
        "earthquake", "landslide", "drought", "fire", "wildfire",
        "ภัยพิบัติ", "เหตุภัยพิบัติ", "เหตุฉุกเฉิน",
        "น้ำท่วม", "อุทกภัย", "พายุ", "แผ่นดินไหว", "ดินถล่ม",
        "ไฟไหม้", "ไฟป่า", "ภัยแล้ง",
    ],    
    [
        "apply", "application", "request", "submit",
        "register", "registration", "form", "document",
        "ยื่นคำขอ", "ยื่นเรื่อง", "ขอรับความช่วยเหลือ",
        "ลงทะเบียน", "สมัคร", "คำร้อง", "แบบคำขอ",
        "เอกสาร", "หลักฐาน", "แนบเอกสาร",
    ],
    [
        "assistance", "aid", "relief", "support", "benefit",
        "grant", "compensation", "payment",
        "ความช่วยเหลือ", "การช่วยเหลือ", "เงินช่วยเหลือ",
        "เงินเยียวยา", "เงินชดเชย", "เงินสงเคราะห์",
        "สิทธิ", "สิทธิ์", "สวัสดิการ",
    ],
    [
        "status", "tracking", "progress", "review", "approve", "approved",
        "reject", "rejected", "pending",
        "สถานะ", "สถานะคำขอ", "ตรวจสอบสถานะ",
        "อยู่ระหว่างพิจารณา", "ผ่านการอนุมัติ", "อนุมัติแล้ว",
        "ไม่ผ่าน", "ถูกปฏิเสธ", "รอผล",
    ],
    [
        "government", "authority", "department", "office",
        "agency", "local authority",
        "หน่วยงาน", "หน่วยงานรัฐ", "ราชการ",
        "กรม", "กระทรวง", "อำเภอ", "จังหวัด",
        "องค์กรปกครองส่วนท้องถิ่น", "เทศบาล", "อบต",
    ],
    # sales / revenue / metrics
    [
        "victim", "affected", "household", "family",
        "damage", "loss", "property damage",
        "ผู้ประสบภัย", "ผู้ได้รับผลกระทบ",
        "ครัวเรือน", "ครอบครัว",
        "ความเสียหาย", "บ้านเสียหาย", "ทรัพย์สินเสียหาย",
    ],
    # time/filter (ช่วยให้คำถาม metric ผ่านง่ายขึ้น)
    [
        "today", "yesterday", "this week", "last week", "this month", "last month",
        "this year", "last year", "date", "time", "range", "between",
        "วันนี้", "เมื่อวาน", "สัปดาห์นี้", "เดือนนี้", "ปีนี้", "ช่วง", "ระหว่าง", "วันที่",
    ],
]

# บางคำมันสั้น/กว้างเกินไป เลยใส่เป็น “ต้อง match แบบคำเต็ม” ลด false positive
_STRICT_WORDS = {
    "aid", "grant", "form", "status", "apply",
    "สิทธิ", "สถานะ", "แบบคำขอ", "คำร้อง",
}

def _normalize(text: str) -> str:
    text = (text or "").strip().lower()
    # ลด noise
    text = re.sub(r"\s+", " ", text)
    return text


def _match_keyword(q: str, kw: str) -> bool:
    if kw in _STRICT_WORDS:
        # match แบบเป็นคำ (word boundary) สำหรับอังกฤษ
        return re.search(rf"\b{re.escape(kw)}\b", q) is not None
    # ไทยไม่มี word boundary ชัด เลยใช้ contains ได้
    return kw in q


def check_in_domain(user_prompt: str) -> DomainGuardResult:
    """
    อนุญาตเฉพาะคำถามเกี่ยวกับ: คำสั่งซื้อ / สาขา / การขาย-รายได้
    ถ้านอกโดเมน -> allowed=False พร้อมข้อความบอกให้ถามใหม่ในกรอบ
    """
    q = _normalize(user_prompt)

    matched: List[str] = []
    for group in _KEYWORD_GROUPS:
        for kw in group:
            if _match_keyword(q, kw):
                matched.append(kw)

    # เกณฑ์ง่ายๆ: มีคีย์เวิร์ดโดเมนอย่างน้อย 1 คำ -> ผ่าน
    if matched:
        return DomainGuardResult(
            allowed=True,
            message="",
            matched_keywords=sorted(set(matched)),
        )

    # ไม่ผ่าน -> ปฏิเสธ + ชี้ทางกลับเข้าโดเมน
    msg = (
        "### ขอโทษด้วยนะครับ 🙏\n"
        "ตอนนี้ระบบรองรับเฉพาะคำถามเกี่ยวกับ **การยื่นคำขอและการช่วยเหลือในช่วงภัยพิบัติ** เท่านั้น\n"
        "คำถามที่ส่งมายังอยู่นอกขอบเขตข้อมูลที่ระบบเข้าถึงได้ครับ\n\n"
        "#### ลองถามใหม่ในลักษณะนี้ดูนะครับ:\n"
        "- ภัยพิบัติเกิดที่ไหนบ้าง\n"
        "- มีผู้ประสบภัยที่ไหนบ้าง\n"
        "- ผู้ประสบภัยมากที่สุดที่จังหวัดใด\n"
        "- สรุปสถานะคำขอความช่วยเหลือให้ฉันหน่อย\n"
    )

    return DomainGuardResult(allowed=False, message=msg, matched_keywords=[])
